...
@pytest.fixture
def mock_chroma_cloud_client():
    with patch('chromadb.CloudClient') as mock_client:
        yield mock_client
...
def test_chroma_db_init_http(mock_chroma_http_client):
    host = "localhost"
    port = 8000
    ssl = False
    headers = {"Authorization": "Bearer token"}
    
    db = ChromaDb(host=host, port=port, ssl=ssl, headers=headers)
    
    mock_chroma_http_client.assert_called_once_with(host="http://" + host, port=port, ssl=ssl, headers=headers)
    assert db.client == mock_chroma_http_client.return_value

def test_chroma_db_init_cloud(mock_chroma_cloud_client):
    api_key = "test_key"
    tenant = "test_tenant"
    database = "test_db"
    
    db = ChromaDb(api_key=api_key, tenant=tenant, database=database)
    
    mock_chroma_cloud_client.assert_called_once_with(api_key=api_key, tenant=tenant, database=database)
    assert db.client == mock_chroma_cloud_client.return_value
...
def test_query_by_embedding(chroma_db_instance, mock_chroma_client, mock_collection):
    ...
    expected_raw_result = {"ids": [["id1"]], "documents": [["doc1"]], "metadatas": [[{"source": "test"}]], "embeddings": [[ [0.1, 0.2] ]]}
    expected_doc_result = [RetrievalDoc(id="id1", text="doc1", metadata={"source": "test"}, embedding=[0.1, 0.2])]

    mock_collection.query.return_value = expected_raw_result
    ...
    assert result == expected_doc_result
...
def test_query_by_text(chroma_db_instance, mock_chroma_client, mock_collection):
    ...
    expected_raw_result = {"ids": [["id3"]], "documents": [["doc3"]], "metadatas": [[{"source": "another_test"}]], "embeddings": None}
    expected_doc_result = [RetrievalDoc(id="id3", text="doc3", metadata={"source": "another_test"}, embedding=None)]
